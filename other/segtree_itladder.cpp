...
void dolazy(int id, int l, int r)
{
	...
	for (int i = id*2; i <= id*2+1; i++)
	{
		...
		sg[i] = (sg[i] + lazy1[id]*k%MOD + k*kk/2%MOD*lazy2[id]%MOD)%MOD; 
...
void update(int id, int l, int r, int u, int v, int val_a, int val_b)
{
	if (r < u || l > v) return;
	if (u <= l && r <= v)
	{
		long long k = r - l + 1, kk = r + l;
		long long val = (val_b - 1ll*val_a*u%MOD + MOD)%MOD; 
		sg[id] = (sg[id] + val*k%MOD + k*kk/2%MOD*(1ll*val_a)%MOD)%MOD; 
