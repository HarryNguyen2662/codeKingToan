...
BigNum find(long long p, long long q, int ti)
{
	if (q == p)
	{
		if (ti == 0) return 0; else return 1;
	}

	long long w = gcd(p,q); p/=w; q/=w;
	if (ti == 0)
	{
		if (p > q)
		{
			 BigNum  re = find(p - q, q, 0);
			if (re < 0) return -1; else return re + re + 2;
		}
		else
		{

			BigNum re = find(q - p, p, 1);
			if (re <= 0) return -1;
			return re + re - 1;
		}
	}
	else
	{
		if (p > q)
		{
			BigNum re = find(p - q, q, 1);
			if (re <= 0) return -1;
			return re + re;
		}
		else
		{
			BigNum re = find(q - p, p, 0);
			if (re < 0) return -1; else return re + re + 3;
		}
	}
}
